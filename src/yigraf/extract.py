...
def _retracted_tasks(local: Sequence[Assertion]) -> Callable[[Assertion], bool]:
    """The workspace's scope for :func:`yigraf.artifacts.retracted_tasks`: only a plan THIS WORKSPACE
    holds speaks for its own contents, so a teammate-only plan and every task under it arrive whole."""
    return artifacts.retracted_tasks(local)
