...
#: Prefix of the version stamp written into the generated SKILL.md, and read back by
#: :func:`installed_skill_version`. An upgrade replaces the CLI and leaves this file exactly as it was,
#: so a 1.5.1 skill happily describes a 1.7.1 CLI — telling its reader that a task cannot be closed at
#: all, months after `close` shipped (feedback-v5 D). The file could not report that, because it
#: carried no claim about which yigraf wrote it. Now it does, in an HTML comment: invisible in every
#: rendered view, outside the frontmatter (so no host has to accept an unknown key), and one cheap
#: line to read.
SKILL_STAMP = "<!-- yigraf-skill-version: "

#: The skill body. NOT an f-string on purpose — it is 20k characters of prose full of `{`/`}` in code
#: samples, and making it one would turn every brace into a format placeholder. The version stamp is
#: prepended by :func:`skill_text` instead.
_SKILL_BODY = """\
---
name: yigraf
...
def skill_text() -> str:
    """The SKILL.md yigraf writes, stamped with the version that generated it."""
    from yigraf import __version__
    head, _, rest = _SKILL_BODY.partition("---\n\n")
    return f"{head}---\n{SKILL_STAMP}{__version__} -->\n\n{rest}"


def installed_skill_version(root: Path) -> str | None:
    """The yigraf version that wrote ``.claude/skills/yigraf/SKILL.md``, or ``None``.

    ``None`` covers three different things on purpose — no skill installed, a skill written before the
    stamp existed, an unreadable file — because the caller does the same thing in all three: say
    nothing. Only a stamp that is *present and different* is worth a nudge; guessing that an unstamped
    file is stale would fire once for every user on the release that introduces stamping, which is the
    kind of false alarm that teaches people to ignore the real one.

    Reads only the first 4 KB: the stamp is on line 5, and a statusline may run this on every refresh.
    """
    path = Path(root) / ".claude" / "skills" / "yigraf" / "SKILL.md"
    try:
        with path.open("r", encoding="utf-8", errors="replace") as fh:
            head = fh.read(4096)
    except OSError:
        return None
    _, marker, rest = head.partition(SKILL_STAMP)
    if not marker:
        return None
